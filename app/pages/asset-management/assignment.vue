...
const statuses = ['Assigned', 'Returned', 'Overdue', 'Cancel'];
...
// ---- modal "Edit Assignment" (hanya field yang bisa berubah) ----
...
function openEditDialog(item: (typeof localAssignments.value)[number]) {
  editingId.value = item.assignmentId;
  editForm.value = {
    status: item.status,
...
async function submitEditForm() {
  ...
  const target = localAssignments.value.find((a) => a.assignmentId === editingId.value);
  if (target) {
    target.status = editForm.value.status as 'Assigned' | 'Returned' | 'Overdue';
    target.condition = editForm.value.condition as 'Good' | 'Fair' | 'Damaged';
    // Return date hanya relevan kalau status Returned, selain itu dikosongkan
    target.returnDate = editForm.value.status === 'Returned' ? editForm.value.returnDate : null;
  }
...
<template>
  <div class="page-wrap">
    ...
    <!-- Modal: Edit Assignment (hanya field yang bisa berubah) -->
    <VDialog v-model="editDialogOpen" max-width="480">
      <VCard rounded="lg">
        <VCardTitle class="d-flex align-center justify-space-between pa-4">
          <span class="text-h6 font-weight-bold">Edit Assignment</span>
          <VBtn
            icon="mdi-close"
            variant="text"
            density="comfortable"
        ...
        <VCardText class="pa-4">
          <VForm ref="editFormRef">
            <VRow>
              ...
              <VCol v-if="editForm.status === 'Returned'" cols="12">
                <VTextField
        ...
        <VCardActions class="pa-4">
          <VSpacer />
          <VBtn variant="outlined" @click="editDialogOpen = false">Cancel</VBtn>
          <VBtn color="primary" @click="submitEditForm">Save</VBtn>
        </VCardActions>
      </VCard>
    </VDialog>
  </div>
