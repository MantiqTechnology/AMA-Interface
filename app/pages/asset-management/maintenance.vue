...
const statuses = ['Open', 'In Progress', 'Completed', 'Waiting Sparepart', 'Cancel'];
...
// ---- inline edit: priority & status langsung dari tabel ----
function updateOrderField(workOrder: string, field: 'priority' | 'status', value: string) {
  const target = localOrders.value.find((m) => m.workOrder === workOrder);
  if (target) {
    // @ts-expect-error - field dibatasi ke 'priority' | 'status' lewat parameter
    target[field] = value;
  }
}
...
async function submitForm() {
  ...
  localOrders.value.unshift({
    ...
    status: form.value.status as 'Open' | 'In Progress' | 'Completed' | 'Waiting Sparepart'
  });
  ...
  <div class="page-wrap">
    ...
    <VCard border rounded="lg" elevation="0">
      ...
      >
        ...
        <!-- Priority: klik chip untuk ubah langsung -->
        <template #[`item.priority`]="{ item }">
          <VMenu>
            <template #activator="{ props: menuProps }">
              <span v-bind="menuProps" class="editable-chip">
                <StatusChip :status="item.priority" />
                <VIcon icon="mdi-chevron-down" size="14" class="ml-1 chip-caret" />
              </span>
            </template>
            <VList density="compact" min-width="140">
              <VListItem
                ...
                :key="p"
                @click="updateOrderField(item.workOrder, 'priority', p)"
              >
                <StatusChip :status="p" />
              </VListItem>
            </VList>
        ...
        <!-- Status: klik chip untuk ubah langsung -->
          ...
          <VMenu>
            ...
            <VList density="compact" min-width="180">
              <VListItem
                v-for="s in statuses"
                :key="s"
                @click="updateOrderField(item.workOrder, 'status', s)"
              >
                <StatusChip :status="s" />
              </VListItem>
            </VList>
