...
const statuses = ['Requested', 'Approved', 'In Transit', 'Completed', 'Rejected'];
...
// ---- modal "Edit Movement" (hanya field yang bisa berubah) ----
...
function openEditDialog(item: (typeof localMovements.value)[number]) {
  editingId.value = item.movementId;
  editForm.value = {
    status: item.status,
...
async function submitEditForm() {
  ...
  const target = localMovements.value.find((m) => m.movementId === editingId.value);
  if (target) {
    target.status = editForm.value.status as typeof target.status;
    target.approvedBy = editForm.value.approvedBy || '-';
  }
...
<template>
  <div class="page-wrap">
    ...
    <VDialog v-model="editDialogOpen" max-width="480">
      <VCard rounded="lg">
        <VCardTitle class="d-flex align-center justify-space-between pa-4">
          <span class="text-h6 font-weight-bold">Edit Movement</span>
          <VBtn
            icon="mdi-close"
            variant="text"
            density="comfortable"
