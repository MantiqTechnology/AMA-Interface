<template>
  ...
  <VContainer fluid class="pt-0">
    <VRow>
      ...
      <VCol cols="12" md="4">
        <SmsKpiCard title="Response Readiness" value="94%" icon="mdi-percent" color="primary" target="Target: 100%" />
      </VCol>
        ...
        <VCard border class="pa-4 h-100">
          ...
          <div class="d-flex flex-column ga-3">
            <div v-for="event in emergencyEvents" :key="event.label" class="d-flex align-center">
              ...
              <VProgressLinear :model-value="(event.value / 8) * 100" color="error" height="12" rounded class="mx-3" />
      ...
      <VCol cols="12">
        <VCard border class="pa-4">
          <SmsTrendChart title="Emergency / Response Trend (6 Months)" v-bind="emergencyTrend" height="280" />
          <div class="text-caption text-medium-emphasis text-center mt-2 italic">* Illustrative mock data based on recent occurrences</div>
        </VCard>
      </VCol>
      ...
      <VCol cols="12" md="6">
        <VCard border class="h-100 d-flex flex-column">
          ...
          <div class="flex-grow-1 overflow-y-auto pa-2" style="max-height: 250px;">
            <VList density="compact" class="pa-0">
              <VListItem v-for="action in postEventActions" :key="action.task" class="px-2 py-1 mb-1 rounded hover-bg">
                <template v-slot:prepend>
                ...
                <template v-slot:append>
                  <VBtn size="small" variant="outlined" color="primary" density="compact" class="text-none">Update</VBtn>
                </template>
              </VListItem>
            </VList>
          </div>
