<template>
  <VContainer fluid class="pb-0">
    ...
    <VCard border class="pa-3 mb-4">
      <div class="d-flex align-center flex-wrap ga-3">
        ...
        <VBtn variant="outlined" color="primary" density="compact" prepend-icon="mdi-download" class="text-none mr-2">
          Export Report
        </VBtn>
        <VBtn color="primary" variant="elevated" prepend-icon="mdi-presentation" class="text-none font-weight-bold shadow-lg" density="compact">
          SRB PRESENTATION
        </VBtn>
      </div>
    </VCard>
  ...
  <VContainer fluid class="pt-0">
    <VRow>
      ...
      <VCol cols="12" md="3">
        <SmsKpiCard title="Total Safety Reports" value="142" icon="mdi-file-chart-outline" color="primary" trend="{ icon: 'mdi-arrow-up', text: '+12% vs Last Qtr' }" />
      </VCol>
      <VCol cols="12" md="3">
        <SmsKpiCard title="Incident Rate / 10k Hrs" value="1.8" icon="mdi-airplane-alert" color="warning" target="ALoSP Target: < 2.5" />
      </VCol>
      <VCol cols="12" md="3">
        <SmsKpiCard title="Avg CAPA Closure (Days)" value="18" icon="mdi-timer-sand" color="info" trend="{ icon: 'mdi-arrow-down', text: 'Improved (Target 21)' }" />
      </VCol>
      <VCol cols="12" md="3">
        <SmsKpiCard title="Safety Culture Score" value="4.2/5" icon="mdi-star-circle" color="success" target="Based on latest survey" />
      </VCol>
        ...
        <VCard border class="h-100 d-flex flex-column">
          <div class="pa-4 pb-2 border-b bg-grey-lighten-5 d-flex justify-space-between align-center">
            <div>
              ...
              <div class="text-caption text-medium-emphasis">Tracking against Acceptable Level of Safety Performance (ALoSP)</div>
            </div>
              ...
              <tbody>
                <tr v-for="spi in spiList" :key="spi.id" class="hover-bg">
                  ...
                  <td class="text-center px-2">
                    <VChip size="small" :color="spi.color" class="font-weight-bold" variant="flat">{{ spi.current }}</VChip>
                  </td>
                  <td class="px-4">
                    <div class="d-flex align-center">
                      <VProgressLinear :model-value="(spi.currentValue / spi.alertValue) * 100" :color="spi.color" height="6" rounded class="mr-2" />
      ...
      <VCol cols="12" md="4">
        <VCard border class="h-100 d-flex flex-column pa-4">
          ...
          <div class="flex-grow-1 d-flex flex-column justify-center align-center">
            ...
            <div class="risk-matrix">
              ...
              <div class="grid-container">
                <div v-for="cell in riskGrid" :key="cell.id" class="risk-cell d-flex justify-center align-center font-weight-bold" :class="cell.colorClass">
          ...
          <div class="d-flex justify-space-between mt-4 px-2">
            <div class="d-flex align-center"><VIcon icon="mdi-circle" color="success" size="12" class="mr-1"/> <span class="text-caption">Low (78)</span></div>
            <div class="d-flex align-center"><VIcon icon="mdi-circle" color="warning" size="12" class="mr-1"/> <span class="text-caption">Med (12)</span></div>
            <div class="d-flex align-center"><VIcon icon="mdi-circle" color="error" size="12" class="mr-1"/> <span class="text-caption">High (2)</span></div>
          </div>
        </VCard>
      </VCol>
      ...
      <VCol cols="12" md="6">
        <VCard border class="pa-4 h-100 bg-blue-grey-lighten-5">
          <div class="d-flex align-center mb-4">
            ...
            <div>
              <div class="text-subtitle-2 font-weight-bold text-blue-grey-darken-3">Pre-Flight Risk (FRAT) Anomalies</div>
              <div class="text-caption text-medium-emphasis">Flights released with High/Red Risk Score (Overridden)</div>
            </div>
          </div>
          
          <VList density="compact" class="bg-transparent pa-0">
            <VListItem v-for="frat in overrideTrends" :key="frat.route" class="px-2 py-2 mb-2 rounded hover-bg bg-white border">
              ...
              <VListItemSubtitle class="text-caption mt-1">Avg Score: <strong>{{ frat.avgScore }}</strong> | Overrides: <strong>{{ frat.count }}x</strong></VListItemSubtitle>
          ...
          <div class="text-center mt-2">
            <VBtn variant="text" size="small" color="primary" class="text-none">Analyze FRAT Data</VBtn>
          </div>
        </VCard>
      </VCol>
