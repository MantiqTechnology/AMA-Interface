...
const statuses = ['All', 'Draft', 'Invitation Sent', 'Quotation Received', 'Evaluation', 'Awarded', 'Cancelled'];
...
const filteredRows = computed(() =>
  rows.value.filter((r) => {
    const matchesSearch = !search.value || r.reference.toLowerCase().includes(search.value.toLowerCase()) || r.itemService.toLowerCase().includes(search.value.toLowerCase());
...
function generateReference(type: 'RFQ' | 'Tender') {
  ...
  const sameDayCount = rows.value.filter((r) => r.reference.startsWith(`${prefix}-AMA-${yy}${mm}${dd}`)).length;
...
<template>
  <div class="proc-page">
    ...
    >
      <template #actions>
        <VBtn color="primary" variant="flat" prepend-icon="mdi-plus" @click="openCreateDialog('RFQ')">Create RFQ</VBtn>
        <VBtn variant="outlined" prepend-icon="mdi-gavel" @click="openCreateDialog('Tender')">Create Tender</VBtn>
    ...
    <VRow class="mt-4" dense>
      <VCol cols="12" md="5">
        <VTextField v-model="search" density="compact" variant="outlined" hide-details prepend-inner-icon="mdi-magnify" placeholder="Cari reference / item" />
      </VCol>
      <VCol cols="6" md="3.5">
        <VSelect v-model="typeFilter" :items="types" density="compact" variant="outlined" hide-details label="Type" />
      </VCol>
      <VCol cols="6" md="3.5">
    ...
    <VCard border rounded="lg" class="mt-4">
      <VDataTable :headers="headers" :items="filteredRows" item-value="id" :items-per-page="10" density="comfortable">
        <template #item.lowestQuote="{ item }">{{ formatIDRFull(item.lowestQuote) }}</template>
      ...
      <VCard v-if="selected" rounded="lg">
        ...
        <VCardText>
          ...
          <table class="simple-table">
            <thead>
              <tr>
                <th>Vendor</th><th>Quoted Price</th><th>Lead Time</th><th>Payment Terms</th><th>Warranty</th><th>Documents</th><th>Score</th><th></th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="q in selected.quotations" :key="q.vendor" :class="{ 'row-highlight': q.selected }">
                <td>{{ q.vendor }}</td>
                <td>{{ formatIDRFull(q.quotedPrice) }}</td>
              ...
              <tr v-if="!selected.quotations.length">
                <td colspan="8" class="text-center text-medium-emphasis py-4">No quotations submitted yet.</td>
              </tr>
            </tbody>
          </table>
        </VCardText>
        ...
        <VCardText style="max-height: 60vh">
          <VForm ref="createFormRef">
            ...
            <VRow dense>
              <VCol cols="12" md="6">
                <VTextField
                  ...
                  density="comfortable"
                  readonly
                  :prepend-inner-icon="createType === 'RFQ' ? 'mdi-file-document-edit-outline' : 'mdi-gavel'"
                />
              </VCol>
              <VCol cols="12" md="6">
                <VSelect
            ...
            <VRow dense>
              ...
              <VCol cols="12" md="6">
                <VMenu v-model="deadlineMenu" :close-on-content-click="false" location="bottom start">
