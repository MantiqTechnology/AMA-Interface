...
const filteredRows = computed(() =>
  rows.value.filter((r) => {
    const matchesSearch =
      !search.value ||
      r.poNumber.toLowerCase().includes(search.value.toLowerCase()) ||
      r.supplier.toLowerCase().includes(search.value.toLowerCase());
    ...
    <VCard border rounded="lg" class="mt-4">
      ...
      >
        <template #item.totalAmount="{ item }">
          {{ formatProcurementIDRFull(item.totalAmount) }}
        </template>
      ...
      <VCard v-if="selected" rounded="lg">
        ...
        <VCardText>
          <VRow dense>
            ...
            <VCol cols="6">
              <div class="detail-row">
                <span>Source PR</span><span>{{ selected.sourcePr }}</span>
              </div>
            </VCol>
            <VCol cols="6">
              <div class="detail-row">
                <span>Total Amount</span><span>{{ formatProcurementIDRFull(selected.totalAmount) }}</span>
              </div>
            </VCol>
            <VCol cols="6">
              <div class="detail-row">
                <span>Expected Delivery</span><span>{{ selected.expectedDelivery }}</span>
              </div>
            </VCol>
          </VRow>
          <VDivider class="my-3" />
          ...
          <VProgressLinear
            :model-value="(selected.delivery.received / selected.delivery.ordered) * 100"
            height="8"
            rounded
            color="primary"
          />
        ...
        <VCardActions class="px-4 pb-4">
          ...
          <VBtn
            ...
            color="primary"
            variant="flat"
            @click="handleSendToVendor(selected)"
          >
            Send to Vendor
          </VBtn>
