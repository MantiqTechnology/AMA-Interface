...
import { formatIDR, statusColor, useProcurementSnackbar } from '../../composables/useProcurement';
import type { Supplier, ProcurementCategory } from '../../types/procurement';
...
function exportCsv() {
  ...
  const body = filteredRows.value
    .map((r) => `${r.vendorCode},${r.company},${r.category},${r.avlStatus},${r.performanceScore},${r.certificateExpiry}`)
    .join('\n');
...
<template>
  <div class="proc-page">
    ...
    <Teleport to="body">
      <Transition name="banner-fade">
        <div v-if="showSuccessBanner" class="success-banner">
          ...
          <button class="success-banner__close" @click="showSuccessBanner = false" aria-label="Tutup notifikasi">
    ...
    >
      <template #actions>
        <VBtn color="primary" variant="flat" prepend-icon="mdi-plus" @click="addDialog = true">Add Supplier</VBtn>
    ...
    <VRow class="mt-4" dense>
      <VCol cols="12" md="4">
        <VTextField v-model="search" density="compact" variant="outlined" hide-details prepend-inner-icon="mdi-magnify" placeholder="Cari vendor / kode" />
      </VCol>
      <VCol cols="6" md="4">
        <VSelect v-model="categoryFilter" :items="categories" density="compact" variant="outlined" hide-details label="Vendor Category" />
      </VCol>
      <VCol cols="6" md="4">
        <VSelect v-model="avlFilter" :items="avlStatuses" density="compact" variant="outlined" hide-details label="AVL Status" />
      </VCol>
    </VRow>
      ...
      <VDataTable :headers="headers" :items="filteredRows" item-value="id" :items-per-page="10" density="comfortable">
        <template #item.performanceScore="{ item }">
          <div class="d-flex align-center" style="gap: 8px; min-width: 110px">
            <VProgressLinear :model-value="item.performanceScore" height="6" rounded color="teal" style="max-width: 70px" />
        ...
        <template #item.avlStatus="{ item }">
          <VChip :color="statusColor(item.avlStatus)" size="small" variant="tonal">{{ item.avlStatus }}</VChip>
        </template>
    ...
    <VNavigationDrawer v-model="drawer" location="right" width="420" temporary>
      <div v-if="selectedSupplier" class="pa-5">
        ...
        <VChip :color="statusColor(selectedSupplier.avlStatus)" size="small" variant="tonal" class="mb-4">
          {{ selectedSupplier.avlStatus }}
        </VChip>

        <div class="detail-block">
          <div class="detail-block__title">General Information</div>
          <div class="detail-row"><span>Category</span><span>{{ selectedSupplier.category }}</span></div>
          <div class="detail-row"><span>Business License</span><span>{{ selectedSupplier.businessLicense }}</span></div>
          <div class="detail-row"><span>Address</span><span>{{ selectedSupplier.address }}</span></div>
        </div>

        <div class="detail-block">
          <div class="detail-block__title">Contact</div>
          <div class="detail-row"><span>Name</span><span>{{ selectedSupplier.contactName }}</span></div>
          <div class="detail-row"><span>Email</span><span>{{ selectedSupplier.contactEmail }}</span></div>
          <div class="detail-row"><span>Phone</span><span>{{ selectedSupplier.contactPhone }}</span></div>
        </div>

        <div class="detail-block">
          <div class="detail-block__title">Aviation Certification</div>
          <div class="detail-row"><span>Certificate</span><span>{{ selectedSupplier.certificateName }}</span></div>
          <div class="detail-row"><span>Expiry</span><span>{{ selectedSupplier.certificateExpiry }}</span></div>
        </div>

        <div class="detail-block">
          <div class="detail-block__title">Commercial</div>
          <div class="detail-row"><span>Payment Terms</span><span>{{ selectedSupplier.paymentTerms }}</span></div>
          <div class="detail-row"><span>Lead Time</span><span>{{ selectedSupplier.leadTimeDays }} days</span></div>
          <div class="detail-row"><span>Performance Score</span><span>{{ selectedSupplier.performanceScore }}/100</span></div>
        </div>

        <div class="detail-block">
          <div class="detail-block__title">Procurement History</div>
          <div class="detail-row"><span>Transactions</span><span>{{ selectedSupplier.procurementHistoryCount }}</span></div>
          <div class="detail-row"><span>Total Spend</span><span>{{ formatIDR(selectedSupplier.totalSpend) }}</span></div>
        </div>
      </div>
    </VNavigationDrawer>
