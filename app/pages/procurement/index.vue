...
import { formatIDRFull, statusColor, useProcurementOverview, useProcurementSnackbar } from '../../composables/useProcurement';
...
const departments = ['Semua Departemen', 'MRO', 'Operations', 'General Affairs', 'IT', 'Finance', 'Ground Handling'];
...
const spendSegments = computed(() => withSemanticColor(spendByCategory as any));
...
const spendByCategoryLegend = computed(() =>
  spendSegments.value.map((s: any) => ({
    label: s.category,
    value: `${(s.amount / 1_000_000_000).toFixed(1)}B (${s.percent}%)`,
    color: s.color
  }))
);
...
<template>
  <div class="proc-page">
    <ProcurementPageHeader eyebrow="Procurement Overview" title="Procurement"
      subtitle="Real-time overview of procurement activities and performance.">
      <template #actions>
        <VBtnToggle v-model="period" density="compact" mandatory color="primary" class="period-toggle">
        ...
        <VTextField :model-value="dateRangeLabel" readonly density="compact" variant="outlined" hide-details
          prepend-inner-icon="mdi-calendar-range" style="max-width: 220px" />

        <VSelect v-model="department" :items="departments" density="compact" variant="outlined" hide-details
          style="max-width: 190px" />

        <VBtn color="primary" variant="flat" :loading="isRefreshing" prepend-icon="mdi-refresh" @click="refresh">
          Refresh
        </VBtn>
    ...
    <div class="text-caption text-medium-emphasis mt-3 mb-1">Last updated: {{ displayedLastUpdated }}</div>
        ...
        <VCard border rounded="lg">
          ...
          <VCardText>
            <div class="pipeline">
              <template v-for="(stage, index) in pipeline" :key="stage.status">
                <div class="pipeline__stage">
                  <div class="pipeline__icon" :style="{ background: `${stage.color}1A`, color: stage.color }">
                    <VIcon :icon="stage.icon" size="20" />
                  </div>
                  <div class="pipeline__label">{{ stage.status }}</div>
                  <div class="pipeline__count">{{ stage.count }}</div>
                </div>
                <VIcon v-if="Number(index) < pipeline.length - 1" icon="mdi-arrow-right" size="18"
            ...
            <div class="bottleneck-alert">
              ...
              <VSpacer />
              <NuxtLink to="/procurement/approval-control" class="link">Lihat detail bottleneck →</NuxtLink>
            </div>
          </VCardText>
        </VCard>
      </VCol>
      ...
      <VCol cols="12" lg="5">
        <VCard border rounded="lg" class="h-100 d-flex flex-column">
              ...
              <table class="simple-table">
                ...
                <tbody>
                  <tr v-for="row in pendingApprovalItems" :key="row.poNumber">
                    <td>{{ row.poNumber }}</td>
                    <td>{{ row.supplier }}</td>
                    <td>{{ formatIDRFull(row.amount) }}</td>
          ...
          <VCardActions class="px-4 pb-3">
            <NuxtLink to="/procurement/purchase-orders" class="link">Lihat semua PO tertahan →</NuxtLink>
          </VCardActions>
        </VCard>
      </VCol>
    </VRow>
    ...
    <VRow class="mt-2" dense>
      <VCol cols="12" lg="3" md="6">
        <NuxtLink to="/procurement/vendor-performance" class="card-link">
          <VCard border rounded="lg" class="h-100 clickable-card">
              ...
              <table class="simple-table simple-table--compact">
                ...
                <tbody>
                  <tr v-for="v in topVendorScores" :key="v.vendor">
                    <td>{{ v.vendor }}</td>
                    <td>
                      <VChip :color="ratingColor[v.rating]" size="x-small" variant="tonal" class="font-weight-bold">
                        {{ v.overallScore }}/100
                      </VChip>
                    </td>
                  </tr>
            ...
            <VCardActions class="px-4 pb-3">
              <NuxtLink to="/procurement/vendor-performance" class="link">Lihat semua vendor performance →</NuxtLink>
            </VCardActions>
          </VCard>
        </NuxtLink>
      </VCol>
          ...
          <VCard border rounded="lg" class="h-100 clickable-card">
            ...
            <VCardText>
              <div class="chart-with-legend">
                <ProcurementDonutChart :segments="aslSegments" :center-value="String(totalAvlVendors)"
                  center-label="Total Vendor" />
                <ProcurementChartLegend :items="aslLegendItems" />
              </div>
            </VCardText>
            <VCardActions class="px-4 pb-3">
              <NuxtLink to="/procurement/vendor-performance" class="link">Lihat semua ASL Status →</NuxtLink>
            </VCardActions>
          </VCard>
        </NuxtLink>
      </VCol>
      ...
      <VCol cols="12" lg="3" md="6">
        <VCard border rounded="lg" class="h-100">
          ...
          <VCardText>
            <div class="chart-with-legend">
              <ProcurementDonutChart :segments="deliverySegments" :center-value="String(totalDeliveriesThisWeek)"
    ...
    <VRow class="mt-2" dense>
        ...
        <VCard border rounded="lg" class="h-100 d-flex flex-column">
          ...
          <VCardText class="flex-grow-1 mt-15">
            <div class="spend-summary-3col">
              <div class="spend-col">
                <div class="spend-col__value">IDR {{ (totalSpendThisYear / 1_000_000_000).toFixed(2) }} B</div>
                <div class="spend-col__label">Total Spend</div>
              </div>
              <div class="spend-col spend-col--center">
                <div class="spend-col__value spend-col__value--accent">{{ budgetUtilizationPercent }}%</div>
                <div class="spend-col__label">Budget Realization</div>
              </div>
              <div class="spend-col spend-col--right">
                <div class="spend-col__value">IDR {{ (totalBudgetThisYear / 1_000_000_000).toFixed(2) }} B</div>
                <div class="spend-col__label">Total Budget</div>
              </div>
            </div>

            <div class="spend-progress">
              <VProgressLinear :model-value="budgetUtilizationPercent" height="8" rounded color="primary" />
            </div>
          </VCardText>
          <VCardActions class="px-4 pb-3">
            <NuxtLink to="/procurement/purchase-orders" class="link">Lihat detail budget realization →</NuxtLink>
          </VCardActions>
        </VCard>
      </VCol>
      ...
      <VCol cols="12" lg="4">
        ...
        <VCard border rounded="lg" class="h-100">
          ...
          <VCardText>
            <div class="chart-with-legend">
              <ProcurementDonutChart :segments="spendSegments"
                :center-value="`${(totalSpendThisYear / 1_000_000_000).toFixed(1)}B`" center-label="Total Spend" />
      ...
      <VCol cols="12" lg="4">
        ...
        <NuxtLink to="/procurement/purchase-orders" class="card-link">
          <VCard border rounded="lg" class="h-100 clickable-card">
            ...
            <VCardActions class="px-4 pb-3">
              <NuxtLink to="/procurement/purchase-orders" class="link">Lihat semua vendor →</NuxtLink>
            </VCardActions>
          </VCard>
        </NuxtLink>
      </VCol>
              ...
              <table class="simple-table">
                ...
                <tbody>
                  <tr v-for="row in recentActivity" :key="row.reference">
                    ...
                    <td>{{ formatIDRFull(row.amount) }}</td>
                    <td>
                      <VChip :color="statusColor(row.status)" size="x-small" variant="tonal">{{ row.status }}</VChip>
                    </td>
                  </tr>
                </tbody>
              </table>
