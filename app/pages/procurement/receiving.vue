...
import { formatIDRFull, statusColor, useProcurementSnackbar } from '../../composables/useProcurement';
...
const claimStatusOptions = ['Open', 'Submitted to Vendor', 'Replacement in Progress', 'Credit Note Issued'];
...
function generateClaimNumber() {
  ...
  const sameDayCount = claimRows.value.filter((c) => c.claimNumber.startsWith(`CLM-AMA-${yy}${mm}${dd}`)).length;
  ...
  <div class="proc-page">
    ...
    <VWindow v-model="tab">
      <VWindowItem value="gr">
        <VCard border rounded="lg">
          <VDataTable :headers="grHeaders" :items="grRows" item-value="id" :items-per-page="10" density="comfortable">
            ...
            <template #item.actions="{ item }">
              <VBtn v-if="item.status === 'Pending Inspection'" size="small" variant="text" color="error" @click="openRejectDialog(item)">Reject</VBtn>
              <VBtn v-if="item.status === 'Pending Inspection'" size="small" variant="text" color="primary" @click="acceptReceipt(item)">Accept</VBtn>
            </template>
          </VDataTable>
        </VCard>
      </VWindowItem>
      ...
      <VWindowItem value="claims">
        <VCard border rounded="lg">
          <VDataTable :headers="claimHeaders" :items="claimRows" item-value="id" :items-per-page="10" density="comfortable">
            <template #item.reason="{ item }">{{ item.reason || '-' }}</template>
            <template #item.claimValue="{ item }">{{ item.claimValue != null ? formatIDRFull(item.claimValue) : '-' }}</template>
            <template #item.status="{ item }"><VChip :color="statusColor(item.status)" size="small" variant="tonal">{{ item.status }}</VChip></template>
            <template #item.actions="{ item }">
              <VBtn size="small"
                variant="outlined"
    ...
    <VDialog v-model="docDialog" max-width="420">
      <VCard v-if="docSelectedGr" rounded="lg">
        <VCardItem>
          <VCardTitle class="text-subtitle-1 font-weight-bold">{{ docSelectedGr.grNumber }}</VCardTitle>
    ...
    <VDialog v-model="rejectDialog" max-width="560" persistent scrollable>
      <VCard v-if="rejectTargetGr" rounded="lg">
        <VCardItem>
          <VCardTitle class="text-subtitle-1 font-weight-bold">Reject {{ rejectTargetGr.grNumber }}</VCardTitle>
          <VCardSubtitle>Lengkapi detail klaim untuk {{ rejectTargetGr.supplier }} — data ini akan otomatis muncul di tab Returns &amp; Claims.</VCardSubtitle>
        </VCardItem>
        ...
        <VCardText style="max-height: 60vh">
          <VForm ref="rejectFormRef">
            <VRow dense>
              <VCol cols="12" md="6">
                <VTextField :model-value="rejectTargetGr.poNumber" label="PO Number" variant="outlined" density="comfortable" readonly />
              </VCol>
              <VCol cols="12" md="6">
                <VTextField :model-value="rejectTargetGr.item" label="Item" variant="outlined" density="comfortable" readonly />
              </VCol>
        ...
        <VCardActions class="px-4 py-3">
          ...
          <VBtn color="error" variant="flat" prepend-icon="mdi-close-circle-outline" @click="submitReject">Reject &amp; Create Claim</VBtn>
        </VCardActions>
      </VCard>
    </VDialog>
    ...
    <VDialog v-model="claimDetailDialog" max-width="420">
      <VCard v-if="claimDetailSelected" rounded="lg">
        <VCardItem>
          <VCardTitle class="text-subtitle-1 font-weight-bold">{{ claimDetailSelected.claimNumber }}</VCardTitle>
        ...
        <VCardText>
          <div class="claim-detail__row">
            ...
            <span class="claim-detail__value">{{ claimDetailSelected.rejectedQty }}</span>
          </div>
          <VDivider class="my-3" />
