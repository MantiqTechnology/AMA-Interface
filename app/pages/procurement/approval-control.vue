      ...
      <VWindowItem value="queue">
        <VCard border rounded="lg">
          <VDataTable
            ...
            density="comfortable"
          >
            <template #item.amount="{ item }">{{ formatProcurementIDRFull(item.amount) }}</template>
            <template #item.waitingSince="{ item }">
              {{ item.waitingSince }} ({{ item.waitingDays }}d)
            </template>
            ...
            <template #item.actions="{ item }">
              <template v-if="item.status === 'Pending'">
                <VBtn size="small" variant="text" color="error" @click="handleReject(item)">
                  Reject
                </VBtn>
                <VBtn size="small" variant="text" color="primary" @click="handleApprove(item)">
                  Approve
                </VBtn>
              </template>
            </template>
        ...
        <VCard border rounded="lg">
          <VCardItem>
            ...
            <VCardSubtitle class="text-caption">
              Data dummy demo — bukan kebijakan resmi perusahaan.
            </VCardSubtitle>
          </VCardItem>
      ...
      <VWindowItem value="match">
        <VCard border rounded="lg">
          <VDataTable
            ...
            :items="threeWayMatchRows"
            item-value="id"
            :items-per-page="10"
            density="comfortable"
          >
            <template #item.poAmount="{ item }">
              {{ formatProcurementIDRFull(item.poAmount) }}
            </template>
            <template #item.grAmount="{ item }">
              {{ item.grAmount ? formatProcurementIDRFull(item.grAmount) : '—' }}
            </template>
            <template #item.invoiceAmount="{ item }">
              {{ item.invoiceAmount ? formatProcurementIDRFull(item.invoiceAmount) : '—' }}
            </template>
            <template #item.status="{ item }">
              <VChip :color="statusColor(item.status)" size="small" variant="tonal">
                {{ item.status }}
