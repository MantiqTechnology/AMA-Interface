...
import { formatIDRFull, priorityColor, statusColor, useProcurementActions, useProcurementSnackbar } from '../../composables/useProcurement';
...
const filteredRows = computed(() =>
  rows.value.filter((r) => {
    const matchesSearch = !search.value || r.prNumber.toLowerCase().includes(search.value.toLowerCase()) || r.itemService.toLowerCase().includes(search.value.toLowerCase());
...
<template>
  <div class="proc-page">
    ...
    <Teleport to="body">
      <Transition name="banner-fade">
        <div v-if="showActionBanner" class="action-banner"
          ...
          <VIcon :icon="actionBannerTone === 'error' ? 'mdi-close-circle' : 'mdi-check-circle'" size="18"
    ...
    <ProcurementPageHeader eyebrow="Purchasing Request" title="Purchase Requisition"
    ...
    <VRow class="mt-4" dense>
      <VCol cols="6" sm="3">
        <ProcurementKpi
          :kpi="{ id: 'open', label: 'Open PR', value: String(kpi.open), icon: 'mdi-clipboard-text-outline', iconColor: '#0F4C81' }" />
      </VCol>
      <VCol cols="6" sm="3">
        <ProcurementKpi
          :kpi="{ id: 'pending', label: 'Pending Review', value: String(kpi.pending), icon: 'mdi-clock-outline', iconColor: '#B45309' }" />
      </VCol>
      <VCol cols="6" sm="3">
        <ProcurementKpi
          :kpi="{ id: 'approved', label: 'Approved', value: String(kpi.approved), icon: 'mdi-check-circle-outline', iconColor: '#15803D' }" />
      </VCol>
      <VCol cols="6" sm="3">
        <ProcurementKpi
          :kpi="{ id: 'urgent', label: 'Urgent', value: String(kpi.urgent), icon: 'mdi-alert-outline', iconColor: '#DC2626' }" />
      </VCol>
    </VRow>

    <VRow class="mt-2" dense>
      <VCol cols="12" md="6">
        <VTextField v-model="search" density="compact" variant="outlined" hide-details prepend-inner-icon="mdi-magnify"
          placeholder="Cari PR / item" />
      </VCol>
      <VCol cols="12" md="4">
        <VSelect v-model="statusFilter" :items="statuses" density="compact" variant="outlined" hide-details
          label="Status" />
      </VCol>
    </VRow>
    ...
    <VCard border rounded="lg" class="mt-4">
      <VDataTable :headers="headers" :items="filteredRows" item-value="id" :items-per-page="10" density="comfortable">
        <template #item.estimatedValue="{ item }">{{ formatIDRFull(item.estimatedValue) }}</template>
        <template #item.priority="{ item }">
          <VChip :color="priorityColor(item.priority)" size="small" variant="tonal">{{ item.priority }}</VChip>
        </template>
    ...
    <VDialog v-model="dialog" max-width="640">
      <VCard v-if="selected" rounded="lg">
        ...
        <VCardText>
          <VRow dense>
            <VCol cols="6">
              <div class="detail-row"><span>Requester</span><span>{{ selected.requester }}</span></div>
            </VCol>
            <VCol cols="6">
              <div class="detail-row"><span>Department</span><span>{{ selected.department }}</span></div>
            </VCol>
            <VCol cols="6">
              <div class="detail-row"><span>Estimated Value</span><span>{{ formatIDRFull(selected.estimatedValue)
                  }}</span>
              </div>
            </VCol>
            <VCol cols="6">
              <div class="detail-row"><span>Required Date</span><span>{{ selected.requiredDate }}</span></div>
            </VCol>
            <VCol cols="6">
              <div class="detail-row"><span>Priority</span><span>{{ selected.priority }}</span></div>
            </VCol>
            <VCol cols="6">
              <div class="detail-row"><span>Budget Check</span><span>{{ selected.budgetCheck }}</span></div>
            </VCol>
          </VRow>
          <VDivider class="my-3" />
          ...
          <div v-for="(item, i) in selected.items" :key="i" class="detail-row">
            ...
            <span>{{ formatIDRFull(item.estimatedUnitPrice) }}</span>
          </div>
          <VDivider class="my-3" />
        ...
        <VCardActions class="px-4 pb-4">
          <VSpacer />
          <VBtn v-if="selected.status === 'Submitted' || selected.status === 'Under Review'" variant="outlined"
            color="error" @click="handleReject(selected)">
            Reject
          </VBtn>
          <VBtn v-if="selected.status === 'Submitted' || selected.status === 'Under Review'" color="primary"
            variant="flat" @click="handleApprove(selected)">
            Approve
          </VBtn>
