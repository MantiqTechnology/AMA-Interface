...
watch(
  [records, () => route.query.flightId],
  ([currentRecords, flightId]) => {
    if (typeof flightId !== 'string') return;
    const target = currentRecords.find(
      (record) => record.id === flightId || record.flightId === flightId
    );
...
<template>
  <VContainer class="px-3 py-5 md:px-4" fluid>
    ...
    >
      <template #actions>
        <VBtn prepend-icon="mdi-airplane-alert" to="/flights/readiness" variant="outlined">
          Blocker pre-flight
        </VBtn>
        <VBtn prepend-icon="mdi-refresh" variant="tonal" @click="refresh"> Perbarui </VBtn>
      </template>
    </DsOperationalPageHeader>

    <VAlert class="mb-4" color="info" icon="mdi-information-outline" variant="tonal">
      Workspace ini khusus handoff maintenance pasca-flight untuk kebutuhan closure. Approval
      handoff tidak menerbitkan return-to-service; keputusan tersebut tetap dimiliki Certifying
      Staff.
    </VAlert>
