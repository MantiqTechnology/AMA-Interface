...
import CurrencyFormDialog from './CurrencyFormDialog.vue';
const active = ref<'active' | 'inactive' | 'all'>('active');
const search = ref('');
const dialog = ref(false);
const editing = ref<CurrencyDto | null>(null);
const {
  data: records,
  pending,
  error,
  ...
  'currencies-page',
  () =>
    fetchApi<CurrencyDto[]>('/api/master-data/currencies', {
...
function edit(record: CurrencyDto) {
  editing.value = record;
  dialog.value = true;
}
async function toggle(record: CurrencyDto) {
  await fetchApi<CurrencyDto>('/api/master-data/currencies/' + record.id + '/status', {
    method: 'PATCH',
  ...
  <VContainer class="px-3 py-5" fluid>
    <div class="mb-5 d-flex flex-wrap align-end ga-4">
      <div>
        ...
        <p class="text-text-secondary">Currency codes used by commercial and finance references.</p>
      </div>
          ...
          <VTable>
            ...
            <tbody>
              <tr v-for="record in records" :key="record.id">
                <td>{{ display(record.currencyCode) }}</td>
                <td>{{ display(record.currencyName) }}</td>
                <td>{{ display(record.symbol) }}</td>
                <td>{{ display(record.decimalPlaces) }}</td>
                <td>
                ...
                <td class="text-right">
                  <DsTooltipIconButton
                    ...
                    :to="'/master-data/currencies/' + record.id"
    ...
    </VCard><CurrencyFormDialog v-model="dialog" :record="editing" @saved="refresh" />
  </VContainer>
</template>
