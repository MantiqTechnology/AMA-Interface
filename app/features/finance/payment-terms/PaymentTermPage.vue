...
import PaymentTermFormDialog from './PaymentTermFormDialog.vue';
const active = ref<'active' | 'inactive' | 'all'>('active');
const search = ref('');
const dialog = ref(false);
const editing = ref<PaymentTermDto | null>(null);
const {
  data: records,
  pending,
  error,
  ...
  'payment-terms-page',
  () =>
    fetchApi<PaymentTermDto[]>('/api/master-data/payment-terms', {
...
function edit(record: PaymentTermDto) {
  editing.value = record;
  dialog.value = true;
}
async function toggle(record: PaymentTermDto) {
  await fetchApi<PaymentTermDto>('/api/master-data/payment-terms/' + record.id + '/status', {
    method: 'PATCH',
...
<template>
  <VContainer class="px-3 py-5" fluid>
    <div class="mb-5 d-flex flex-wrap align-end ga-4">
      <div>
        <h1 class="text-h4 font-weight-bold">Payment Terms</h1>
        <p class="text-text-secondary">Payment due-day rules for customer and vendor accounts.</p>
      </div>
          ...
          <VTable>
            ...
            <tbody>
              <tr v-for="record in records" :key="record.id">
                <td>{{ display(record.termCode) }}</td>
                <td>{{ display(record.termName) }}</td>
                <td>{{ display(record.dueDays) }}</td>
                ...
                <td class="text-right">
                  <DsTooltipIconButton
                    ...
                    :to="'/master-data/payment-terms/' + record.id"
    ...
    </VCard><PaymentTermFormDialog v-model="dialog" :record="editing" @saved="refresh" />
  </VContainer>
</template>
