...
const editing = ref<VendorDto | null>(null);
const {
  data: records,
  pending,
  error,
  ...
  'vendors-page',
  () =>
    fetchApi<VendorDto[]>('/api/master-data/vendors', {
...
function edit(record: VendorDto) {
  editing.value = record;
  dialog.value = true;
}
async function toggle(record: VendorDto) {
  await fetchApi<VendorDto>('/api/master-data/vendors/' + record.id + '/status', {
    method: 'PATCH',
  ...
  <VContainer class="px-3 py-5" fluid>
    <div class="mb-5 d-flex flex-wrap align-end ga-4">
      <div>
        ...
        <p class="text-text-secondary">Non-fuel vendors used by operating and finance workflows.</p>
      </div>
          ...
          <VTable>
            ...
            <tbody>
              <tr v-for="record in records" :key="record.id">
                <td>{{ display(record.vendorCode) }}</td>
                <td>{{ display(record.vendorName) }}</td>
                <td>{{ display(record.vendorType) }}</td>
    ...
    </VCard><VendorFormDialog v-model="dialog" :record="editing" @saved="refresh" />
  </VContainer>
</template>
