...
import DgCategoryFormDialog from './DgCategoryFormDialog.vue';
const active = ref<'active' | 'inactive' | 'all'>('active');
const search = ref('');
const dialog = ref(false);
const editing = ref<DgCategoryDto | null>(null);
const {
  data: records,
  pending,
  error,
  ...
  'dg-categories-page',
  () =>
    fetchApi<DgCategoryDto[]>('/api/master-data/dg-categories', {
...
function edit(record: DgCategoryDto) {
  editing.value = record;
  dialog.value = true;
}
async function toggle(record: DgCategoryDto) {
  await fetchApi<DgCategoryDto>('/api/master-data/dg-categories/' + record.id + '/status', {
    method: 'PATCH',
...
<template>
  <VContainer class="px-3 py-5" fluid>
    <div class="mb-5 d-flex flex-wrap align-end ga-4">
      <div>
        <h1 class="text-h4 font-weight-bold">DG Categories</h1>
        <p class="text-text-secondary">
          Dangerous Goods category references for future manifest demo flows.
        </p>
      </div>
          ...
          <VTable>
            <thead>
              <tr>
                ...
                <th>Requires special approval</th>
                <th>Status</th>
                <th />
              </tr>
            </thead>
            <tbody>
              <tr v-for="record in records" :key="record.id">
                <td>{{ display(record.dgCode) }}</td>
                <td>{{ display(record.dgClass) }}</td>
                <td>{{ display(record.description) }}</td>
                <td>{{ display(record.handlingInstruction) }}</td>
                <td>{{ display(record.requiresSpecialApproval) }}</td>
                <td>
                ...
                <td class="text-right">
                  <DsTooltipIconButton
                    ...
                    :to="'/master-data/dg-categories/' + record.id"
    ...
    </VCard><DgCategoryFormDialog v-model="dialog" :record="editing" @saved="refresh" />
  </VContainer>
</template>
