...
const {
  ...
  'agents-page',
  () =>
    fetchApi<AgentDto[]>('/api/master-data/agents', {
...
function edit(record: AgentDto) {
  editing.value = record;
  dialog.value = true;
}
async function toggle(record: AgentDto) {
  await fetchApi<AgentDto>('/api/master-data/agents/' + record.id + '/status', {
    method: 'PATCH',
...
<template>
  <VContainer class="px-3 py-5" fluid>
    <div class="mb-5 d-flex flex-wrap align-end ga-4">
      <div>
        <h1 class="text-h4 font-weight-bold">Agents & Counters</h1>
        <p class="text-text-secondary">Sales agents, station counters, and cargo partners.</p>
      </div>
          ...
          <VTable>
            ...
            <tbody>
              <tr v-for="record in records" :key="record.id">
                <td>{{ display(record.agentCode) }}</td>
                <td>{{ display(record.agentName) }}</td>
                <td>{{ display(record.agentType) }}</td>
                <td>{{ display(record.stationId) }}</td>
                <td>{{ display(record.commissionBasisPoints) }}</td>
                ...
                <td class="text-right">
                  <DsTooltipIconButton
                    ...
                    :to="'/master-data/agents/' + record.id"
    ...
    </VCard><AgentFormDialog v-model="dialog" :record="editing" @saved="refresh" />
  </VContainer>
</template>
