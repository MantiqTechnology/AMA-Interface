...
async function main() {
  ...
  try {
    const custody = createServices(client.sqlite).maintenance.requestAircraftMoveIn(
      input.slotId,
      { idempotencyKey: input.idempotencyKey },
      actor
    );
    console.log(JSON.stringify({ ok: true, custodyId: custody.id, status: custody.status }));
  } catch (error) {
