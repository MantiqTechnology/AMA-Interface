...
import type { SafetyReportDto, SafetyReportInput, SafetyReportListQuery } from './types';
...
export class SafetyReportRepository {
  ...
  async list(query: SafetyReportListQuery): Promise<SafetyReportDto[]> {
    ...
    if (query.search) {
      ...
      conditions.push(
        or(like(safetyReports.reportNumber, term), like(safetyReports.description, term)) as SQL
      );
    }
