...
export class CapaTicketRepository {
  ...
  async list(query: CapaListQuery): Promise<CapaTicketDto[]> {
    ...
    if (query.assignedToUserId) conditions.push(eq(capaTickets.assignedToUserId, query.assignedToUserId));
    if (query.isOverdueEscalated !== undefined) conditions.push(eq(capaTickets.isOverdueEscalated, query.isOverdueEscalated));

    if (query.search) {
      const term = `%${query.search}%`;
      conditions.push(
        or(
          like(capaTickets.ticketNumber, term),
          like(capaTickets.subject, term)
        ) as SQL
      );
    }
