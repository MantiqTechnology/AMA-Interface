...
export class CapaTicketService {
  ...
  async create(input: CapaTicketInput) {
    ...
    try {
      ...
      return await this.repository.create(id, ticketNumber, input, new Date().toISOString());
    } catch (error) {
  ...
  async escalateOverdueTicket(id: string, escalatedToUserId: string) {
    ...
    if (ticket.dueDate >= today) {
      throw new DomainError(
        'CAPA_NOT_OVERDUE',
        'Tiket ini belum melewati batas waktu (due date) sehingga tidak bisa dieskalasi.',
        400
      );
    }
    ...
    if (ticket.status === 'VERIFIED' || ticket.status === 'CLOSED') {
      throw new DomainError(
        'CAPA_ALREADY_CLOSED',
        'Tidak bisa mengeskalasi tiket yang sudah ditutup.',
        400
      );
    }
  ...
  private async validate(input: CapaTicketInput) {
    ...
    if (!input.dueDate) {
      throw new DomainError(
        'CAPA_DUE_DATE_REQUIRED',
        'Due date is mandatory for CAPA tracking.',
        422
      );
    }
  }
