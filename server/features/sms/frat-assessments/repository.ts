...
export class FratAssessmentRepository {
  ...
  async getById(id: string): Promise<FratAssessmentDto | null> {
    const row = await this.db.select().from(fratAssessments).where(eq(fratAssessments.id, id)).get();
  ...
  async create(
    id: string, 
    input: Omit<FratAssessmentDto, 'id' | 'overrideSignoffByUserId' | 'overrideReason' | 'createdAt' | 'updatedAt'>, 
    timestamp: string
  ) {
    const row = await this.db
