...
export class FratAssessmentService {
  ...
  async overrideFrat(id: string, approverId: string, reason: string) {
    ...
    if (!frat.isHardLocked) {
      throw new DomainError('FRAT_NOT_LOCKED', 'FRAT ini tidak dalam status Hard-Lock, tidak perlu di-override.', 400);
    }
    ...
    try {
      const row = await this.repository.overrideHardLock(id, approverId, reason, new Date().toISOString());
  ...
  private validateScores(input: FratAssessmentInput) {
    if (input.weatherRiskScore < 0 || input.weatherRiskScore > 25) {
      throw new DomainError('INVALID_WEATHER_SCORE', 'Weather risk score must be between 0 and 25.', 422);
    }
    if (input.airstripRatingScore < 0 || input.airstripRatingScore > 25) {
      throw new DomainError('INVALID_AIRSTRIP_SCORE', 'Airstrip rating score must be between 0 and 25.', 422);
    }
  }
  ...
  private rethrowWriteError(error: unknown): never {
    ...
    if (message.includes('UNIQUE constraint failed'))
      throw new DomainError('FRAT_DUPLICATE', 'FRAT assessment for this flight already exists.', 409);
