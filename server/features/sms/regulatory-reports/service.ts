...
export class RegulatoryReportService {
  ...
  async generateReport(input: RegulatoryReportInput) {
    ...
    try {
      ...
      return await this.repository.create(id, referenceNumber, input, new Date().toISOString());
    } catch (error) {
  ...
  async submitToAuthority(id: string, receiptNumber?: string) {
    ...
    try {
      const row = await this.repository.markAsSubmitted(
        id,
        receiptNumber ?? null,
  ...
  private rethrowWriteError(error: unknown): never {
    ...
    if (message.includes('UNIQUE constraint failed'))
      throw new DomainError(
        'REGULATORY_REPORT_DUPLICATE',
        'Report reference number already exists.',
        409
      );
