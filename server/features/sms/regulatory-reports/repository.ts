...
export class RegulatoryReportRepository {
  ...
  async list(query: RegulatoryReportListQuery): Promise<RegulatoryReportDto[]> {
    ...
    if (query.reportType) conditions.push(eq(regulatoryComplianceReports.reportType, query.reportType));
  ...
  async create(id: string, referenceNumber: string, input: RegulatoryReportInput, timestamp: string) {
    const row = await this.db
