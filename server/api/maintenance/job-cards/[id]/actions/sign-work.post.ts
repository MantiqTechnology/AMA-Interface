import {
  ...
  maintenanceJobCardWorkSignoffSchema
...
export default defineApiEventHandler(async (event) => {
  ...
  const body = await parseBody(event, maintenanceJobCardWorkSignoffSchema);
  return getServices().maintenance.signWork(id, body, getDemoActorContext(event));
});
