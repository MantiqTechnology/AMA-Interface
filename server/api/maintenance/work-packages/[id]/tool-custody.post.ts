...
import { assignToolCustodySchema } from '#shared/features/maintenance-v21-schemas';
...
export default defineApiEventHandler(async (event) => {
  ...
  const body = await parseBody(event, assignToolCustodySchema);
  return getServices().resourceV21.assignToolCustody(body, getDemoActorContext(event), id);
});
