...
import { assignPersonnelSchema } from '#shared/features/maintenance-v21-schemas';
...
export default defineApiEventHandler(async (event) => {
  ...
  const body = await parseBody(event, assignPersonnelSchema);
  return getServices().resourceV21.assignPersonnel(body, getDemoActorContext(event), id);
});
