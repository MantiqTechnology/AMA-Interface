...
export default defineApiEventHandler(async (event) => {
  ...
  const body = await parseBody(event, createMaintenanceSlotSchema);
  return getServices().maintenance.bookMaintenanceSlot(id, body, getDemoActorContext(event));
});
