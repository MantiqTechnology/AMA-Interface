...
import { allocateToolSchema } from '#shared/features/maintenance-v21-schemas';
...
export default defineApiEventHandler(async (event) => {
  ...
  const body = await parseBody(event, allocateToolSchema);
  return getServices().resourceV21.allocateTool(body, getDemoActorContext(event), id);
});
