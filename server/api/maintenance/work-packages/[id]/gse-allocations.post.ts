...
export default defineApiEventHandler(async (event) => {
  ...
  const body = await parseBody(event, allocateMaintenanceGseSchema);
  return getServices().maintenance.allocateGse(id, body, getDemoActorContext(event));
});
