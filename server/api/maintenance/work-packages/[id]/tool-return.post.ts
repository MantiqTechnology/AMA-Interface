...
import { returnToolSchema } from '#shared/features/maintenance-v21-schemas';
...
export default defineApiEventHandler(async (event) => {
  ...
  const body = await parseBody(event, returnToolSchema);
  return getServices().resourceV21.returnTool(body, getDemoActorContext(event), id);
});
