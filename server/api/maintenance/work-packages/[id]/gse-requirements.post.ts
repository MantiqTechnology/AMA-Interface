import {
  createMaintenanceGseRequirementSchema,
...
export default defineApiEventHandler(async (event) => {
  ...
  const body = await parseBody(event, createMaintenanceGseRequirementSchema);
  return getServices().maintenance.createGseRequirement(id, body, getDemoActorContext(event));
});
