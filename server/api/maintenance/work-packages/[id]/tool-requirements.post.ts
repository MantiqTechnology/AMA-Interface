...
import { createToolRequirementSchema } from '#shared/features/maintenance-v21-schemas';
...
export default defineApiEventHandler(async (event) => {
  ...
  const body = await parseBody(event, createToolRequirementSchema);
  // Inject workPackageId from route param
  const input = { ...body, workPackageId: id };
  return getServices().resourceV21.createToolRequirement(input, getDemoActorContext(event));
});
