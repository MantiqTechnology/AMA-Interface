import {
  addMaintenanceShiftRosterSchema,
...
export default defineApiEventHandler(async (event) => {
  ...
  const body = await parseBody(event, addMaintenanceShiftRosterSchema);
  return getServices().maintenance.addShiftRoster(id, body, getDemoActorContext(event));
});
