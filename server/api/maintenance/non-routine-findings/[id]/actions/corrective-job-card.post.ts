import {
  createCorrectiveJobCardFromFindingSchema,
...
export default defineApiEventHandler(async (event) => {
  ...
  const body = await parseBody(event, createCorrectiveJobCardFromFindingSchema);
  return getServices().maintenance.createCorrectiveJobCardForFinding(
    id,
    body,
