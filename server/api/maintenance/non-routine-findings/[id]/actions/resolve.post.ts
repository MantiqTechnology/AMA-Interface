import {
  ...
  resolveNonRoutineFindingSchema
...
export default defineApiEventHandler(async (event) => {
  ...
  const body = await parseBody(event, resolveNonRoutineFindingSchema);
  return getServices().maintenance.resolveNonRoutineFinding(id, body, getDemoActorContext(event));
});
