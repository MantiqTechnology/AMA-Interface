...
export default defineApiEventHandler(async (event) => {
  ...
  const body = await parseBody(event, assessNonRoutineFindingSchema);
  return getServices().maintenance.assessNonRoutineFinding(id, body, getDemoActorContext(event));
});
