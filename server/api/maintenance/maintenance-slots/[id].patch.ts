import {
  ...
  rescheduleMaintenanceSlotSchema
...
export default defineApiEventHandler(async (event) => {
  ...
  const body = await parseBody(event, rescheduleMaintenanceSlotSchema);
  return getServices().maintenance.rescheduleMaintenanceSlot(id, body, getDemoActorContext(event));
});
