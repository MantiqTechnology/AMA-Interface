import { z } from 'zod';
import { updateStationCostBodySchema } from '#shared/contracts/flight-operations';
...
export default defineApiEventHandler(async (event) => {
  ...
  const body = await parseBody(event, updateStationCostBodySchema);
  ...
  return service.updateStationCost(id, body, getDemoActorId(event));
});
