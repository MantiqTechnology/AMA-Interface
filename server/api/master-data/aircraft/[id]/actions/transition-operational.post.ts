import {
  ...
  aircraftOperationalTransitionSchema
...
export default defineApiEventHandler(async (event) => {
  requireDemoPermission(event, 'aircraft.lifecycle.manage');
  const { id } = parseParams(event, aircraftIdParamsSchema);
  const body = await parseBody(event, aircraftOperationalTransitionSchema);
  return getServices().aircraftAirworthiness.transitionOperational(id, body, {
