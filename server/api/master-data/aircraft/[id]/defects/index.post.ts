...
export default defineApiEventHandler(async (event) => {
  requireDemoPermission(event, 'aircraft.defect.report');
  const { id } = parseParams(event, aircraftIdParamsSchema);
  const body = await parseBody(event, aircraftDefectInputSchema);
  return getServices().aircraftAirworthiness.reportDefect(id, body, {
