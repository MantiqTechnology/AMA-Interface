...
export const regulatoryComplianceReports = sqliteTable(
  'regulatory_compliance_reports',
  {
    ...
    status: text('status')
      .$type<'DRAFT' | 'GENERATED' | 'PENDING_APPROVAL' | 'SUBMITTED' | 'ACKNOWLEDGED' | 'NEED_REVISION'>()
      .notNull()
      .default('DRAFT'),
...
export const companyCertificates = sqliteTable(
  'company_certificates',
  {
    ...
    status: text('status')
      .$type<'VALID' | 'EXPIRING_SOON' | 'EXPIRED'>()
      .notNull()
      .default('VALID'),
...
export const operationsManuals = sqliteTable(
  'operations_manuals',
  {
    id: text('id').primaryKey(),
...
export const authorityCorrespondences = sqliteTable(
  'authority_correspondences',
  {
    id: text('id').primaryKey(),
    ...
    status: text('status')
      .$type<'UNREAD' | 'READ' | 'ACTION_REQUIRED'>()
      .notNull()
      .default('UNREAD'),
...
export const safetyGovernancePolicies = sqliteTable(
  ...
  (table) => [
    index('idx_safety_policies_status').on(table.status)
  ]
);
...
export const justCultureAssessments = sqliteTable(
  'just_culture_assessments',
  {
    ...
    actionType: text('action_type')
      ...
      .notNull(),
    isNonPunitiveProtected: integer('is_non_punitive_protected', { mode: 'boolean' }).notNull().default(true),
