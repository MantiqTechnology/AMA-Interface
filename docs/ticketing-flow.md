...
3. `POST /api/ticketing/sales/open` creates a commercial `flight_orders` row and a unique `ticketing_sales` mapping to the OCC flight.
...
3. A unique `(flight_order_id, seat_number)` index prevents concurrent double booking.
4. `passenger_tickets` and `flight_manifest_passengers` are written in one SQLite transaction.
...
5. `cargo_bookings` and `flight_manifest_cargo_items` are written in one transaction. Payment is required before proof of delivery.
